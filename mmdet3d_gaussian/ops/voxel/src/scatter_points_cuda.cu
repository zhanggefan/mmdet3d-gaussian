...
at::Tensor dynamic_point_to_voxel_scatter_reduce_gpu(
  ...
  const int num_voxels = reduce_count.size(0);

  auto reduced_feats = at::empty({num_voxels, num_feats}, feats.options());
...
dynamic_point_to_voxel_scatter_index_gpu(const at::Tensor &coors) {
  CHECK_INPUT(coors);

  const int num_input = coors.size(0);

  if (num_input == 0)
    return {coors.clone().detach(), coors.new_empty({0}, torch::kInt32),
